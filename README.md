<img src="art/FluxBox_TP.png" width="400" alt="FluxBoxLogo">

# FluxBox (v0.2.9-beta)
Made With ❤️ by **Phantomware**
> **Developer:** Wrenchiz  
> **Version:** v0.2.9-beta  
> **Engine:** HTML5 Canvas + Pixi.js  
> **Play Open Beta:** [https://play-fluxbox.netlify.app/](https://play-fluxbox.netlify.app/)  

---

## 🏆 Secret Achievements

| Achievement | How to Unlock |
|-------------|---------------|
| **FluxBox Master** 🌈 | Type "fluxbox" anywhere on the screen |
| **Garbage Man** 🗑️ | Press the 'C' key 10 times |

---

## 🥚 Easter Egg

There's a hidden surprise in the **About** section!  
Click on **"Wrenchiz"** 10 times rapidly and watch what happens 💘.

---

![Phantomware Logo](./art/Phantomware_TP_Logo.png)

# Phantomware Official Website

> **Official Website:** [https://phantomware.netlify.app/](https://phantomware.netlify.app/)

---

## ⚖️ License

This project is licensed under a **Proprietary Source-Available License**.  
Source code is made publicly visible solely for transparency, security auditing, and verification purposes.  
See the [LICENSE](LICENSE.md) file for full terms and conditions.

© 2026 **Phantomware**. All rights reserved.
